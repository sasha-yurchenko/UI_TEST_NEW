...
class GeoModal:
    ...
    def country_selection(self, country, city):
        try:
            self.driver.find_element(*Locators.field_name_countries_xpath).send_keys(country)
            self.app.text_to_be_present_in_element(*Locators.button_check_counties_xpath), country
            self.driver.find_element(*Locators.button_check_counties_xpath).click()
            self.driver.find_element(*Locators.field_name_city_xpath).send_keys(city)
            self.app.text_to_be_present_in_element(*Locators.button_check_city_xpath), city
            self.driver.find_element(*Locators.button_check_city_xpath).click()
            self.driver.find_element(*Locators.submit_geo_xpath).click()
            return True
    ...
    def assert_name_geo(self, city):
        try:
            self.app.text_to_be_present_in_element_value(*Locators.search_wrap_geo_xpath), city
            return True
        except NoSuchElementException:
            return False and self.app.destroy()
    ...
    def click_choose_another_country(self):
        btn_another_country = self.driver.find_element(*Locators.button_choose_another_country)
        print(btn_another_country)
        self.app.element_to_be_clickable(btn_another_country)
        btn_another_country.click()